...
def parse_txt_to_excel(txt_file, df_exist=None):
    ...
    is_end = False
    df = pd.DataFrame(columns=["file", "totel cases", "cases"]) if df_exist is None else df_exist
    line_id = 0
    ...
    while line_id < len(lines):
        ...
        if match:
            ...
            if file_name_num.empty:
                df = pd.concat([df, pd.DataFrame({"file": [file_name],"totel cases": [item_num], "cases": [test_file_num]})], ignore_index=True)
                ...
                try:
                    df.loc[file_name_num[0], "totel cases"] = int(df.loc[file_name_num[0], "totel cases"]) + (item_num-(org_items-new_items))
                except:
                    print("Exception got")
...
def process_path(input_dir, output_file, sheet_title, workflow):
    ...
    if os.path.exists(output_file):
        ...
        df_parsed = pd.DataFrame(columns=["file", "totel cases", "cases"])
    ...
    if os.path.isfile(input_dir):
        ...
        # df_parsed = pd.DataFrame(columns=["file", "totel cases", "cases"])
        configs_dict={
            'b200':'linux-jammy-cuda12.8-py3.10-gcc11-test-b200 _ test',
            #'periodic': 'linux-jammy-cuda12.8-py3.10-gcc11 _ test',
            'periodic': 'linux-jammy-cuda12.8-py3-gcc11-slow-gradcheck _ test',
            'inductor-unittest': 'inductor-test _ test',
            'slow': 'linux-jammy-py3.10-clang12 _ test', 
            'stock_pytorch_xpu': 'linux-jammy-xpu-n-py3.10 _ test ' 
        }
        for file_name in os.listdir(input_dir):
            if file_name.endswith('.txt') and configs_dict[workflow] in file_name:
    ...
    file_summary = df_parsed.iloc[:, :2].dropna(how='all')
    file_summary.to_csv(output_file.split('.')[0] + '.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-w', '--workflow', default='periodic', help='The workflow to parse, periodic, b200, xpu or slow, etc.')
    ...
    process_path(input_dir, output_excel_file, sheet_title, workflow)
